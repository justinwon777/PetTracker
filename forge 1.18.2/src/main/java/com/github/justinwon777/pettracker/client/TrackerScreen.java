    ...
    protected void renderTooltip(PoseStack stack, int x, int y) {
        if (this.teleportButton.isHoveredOrFocused()) {
            ...
            if (this.teleportButton.isActive() || this.trackerList.getSelected() == null) {
                tooltips.add(new TextComponent("Teleports mob to you").withStyle(ChatFormatting.GRAY).withStyle(ChatFormatting.ITALIC));
            } else {
                tooltips.add(new TextComponent("Mob is either dead or in an unloaded chunk. " +
        ...
        if (this.removeButton.isHoveredOrFocused()) {
            List<Component> tooltips = new ArrayList<>();
            tooltips.add(new TextComponent("Removes mob from the list.").withStyle(ChatFormatting.GRAY).withStyle(ChatFormatting.ITALIC));
        ...
        public class Entry extends ObjectSelectionList.Entry<Entry> {
            ...
            public void render(PoseStack pPoseStack, int pIndex, int pTop, int pLeft, int pWidth, int pHeight, int pMouseX, int pMouseY, boolean pIsMouseOver, float pPartialTick) {
                ...
                TrackerScreen.this.font.draw(pPoseStack, this.name,
                        (float)(TrackerList.this.width / 2 - TrackerScreen.this.font.width(name) / 2),
                        (float)(pTop + 1), 4210752);
                TrackerScreen.this.font.draw(pPoseStack, location,
                        (float)(TrackerList.this.width / 2 - TrackerScreen.this.font.width(location) / 2),
